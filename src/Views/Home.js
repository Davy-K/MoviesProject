...
import { Image, ScrollView, Text, View, TouchableOpacity, StyleSheet, FlatList, Dimensions } from 'react-native';
import { Movies } from './MoviesList';
import { fetchBestMovies, fetchFavoriteMovies, fetchMarvelMovies } from '../Providers/callAPI';
...
export const PromoScreen = () => {
  return (
    <View style={{ margin: 10, justifyContent: 'center', alignItems: 'center' }}>
      ...
      <Text style={{ color: '#fff', fontSize: 15, margin: 10 }}>
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum quis pulvinar purus, eu luctus est.
      </Text>
...
export const Item = ({ title, value, category, setCategory }) => (
  ...
  >
    <Text style={[styles.title, category.label === title && styles.activeTitle]}>{title}</Text>
  </TouchableOpacity>
);
    ...
    <ScrollView style={styles.container}>
        <View style={styles.heroContainer}>
            <FlatList
                ...
                renderItem={({ item }) => (
                    <Image source={{ uri: `https://image.tmdb.org/t/p/w500${item?.poster_path}` }} style={styles.heroImage} />
                )}
                keyExtractor={(item) => item.id.toString()}
            />
                            <LinearGradient
                    colors={[gradientColorDark.gradientColor, gradientColorDark.gradientColor2]}
                    start={{ x: 0, y: 1 }}
                    end={{ x: 0, y: 0 }}
            ...
            <View style={styles.heroOverlay}>
                <View style={{marginBottom: 20, flexDirection: 'row', justifyContent: 'center', gap: 105}}>
                    <TouchableOpacity >
